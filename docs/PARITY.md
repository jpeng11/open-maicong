...
| Tag | Meaning |
| :--- | :--- |
| Live verified | Observed and verified on a real G75 V2 receiver (MCU FW 1.14 / RF 1.30, 2026-09-21) via live write, fresh readback, and clean restoration. |
...
| Mac / Win mode | en `670`–`671`; `iG` `toggleMacMode` | Settings OS Mode → `applySettings({ macMode })` FUNC byte 1 | MATCH (live write verified) | Hub ties default layer to Mac; app also maps `macMode&3` to initial layer. | Live write and fresh readback verified (2026-09-21). |
| Preview vs onboard edit | en `unActiveProfileTip*`; `isPreview` in `iX`/`iG` | Load for editing vs Activate; local preview hydrates from disk | MATCH in source (live activate verified) | Load still does not `SET_BASE`. Custom items preview with zero HID. Drag local→onboard is tE. | Mock: load ≠ SET_BASE; local preview zero writes. Live activate verified on hardware (2026-09-21). |
...
| Hub control / action | Vendor evidence | App path | Status | Semantic gap | Verification needed |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 4 onboard slots | en `storedOnDevice`; `rw` profile list | Sidebar onboard list + enable 4th (`enableProfiles`) | MATCH (live write verified) | Drag local onto a slot is tE. Activate/Load remain distinct. | Live write and fresh readback verified (2026-09-21): 4th slot enabled (3->4), active switch (1->0), restored to 3 with order preserved. |
...
| Import / export file | en `15`/`16`; format doc version 3; SDK `se`/`le`/`ue`/`pe`/`ge` | Official v3 KeyboardProfile import creates a **local** item (zero HID) and converts `advancedKeys.mt/tgl` into native MT/TGL tables. Onboard official export serializes parsed native tables as `{clickKey,downKey}` / `{type,code1,code2}`. Native backup JSON on Backup tab retained. | MATCH (live apply & roundtrip verified for v3/native) | Cloud share-codes and lighting-only envelopes remain missing. Official JSON has no keyExtras field; DKS is rejected on G75 mechanical. Nested malformed official/local objects fail closed (no replacement defaults). Consecutive official times are preserved as zero-code delays. Writes remain non-atomic. | Verified on hardware (2026-09-21): native JSON and official v3 export, import, live apply, and re-export match all 4 layers, 83 key LEDs, settings, and macros. Native preserves more profile fields (each export backs up one profile, not the whole keyboard); official format omits calibration/keyExtras and preserves live hardware values on apply. Writes remain non-atomic. |
...
| Hub control / action | Vendor evidence | App path | Status | Semantic gap | Verification needed |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 23 built-in main effects 0–22 | G75 catalog and GLW resolver; `G75_LIGHTING_SCOPE.md` | 22 preset tiles plus Custom/Local still and GIF libraries | MATCH (live write verified for presets 0..22 & custom) | Vendor order, names and selection states match. Local still create/select/rename/delete, 83-zone still edits, GIF import/edit/playback, and selectedLightEffect 241/242 offset 840/56 are implemented. Cloud/share/side still remain missing. | Live write and readback verified across IDs 0..22 and supported fields (2026-09-21); user visually verified red constant color. (Visual inspection of all animation patterns not claimed; physical CMD 242 / CMD 221 unproven). |
...
| Brightness 0–100 | en `256`; FUNC byte 9 | slider 0–100 | MATCH (live write verified) | | Live read; live write and readback verified (2026-09-21). |
| Speed 0–4 | G75 catalog capability table in `G75_LIGHTING_SCOPE.md` | Slider0–4, effect-specific visibility and guards | MATCH (live write verified) | Hidden for main0,2,3 and side Constant On/Off. | Independent 27-row fixture tested; live write and readback verified (2026-09-21). |
| Direction | G75 GLW direction families in `G75_LIGHTING_SCOPE.md` | Four effect-specific label pairs, values0/1 | MATCH (live write verified) | Only main6,7,8,10 expose direction. | Independent fixture tested; live write and readback verified (2026-09-21). |
| Solid custom color on/off + hex | FUNC customColorDisabled + RGB | checkbox + color + presets | MATCH (live write verified) | Hub also has **palette / still / GIF libraries**. | Live write and readback verified (2026-09-21). |
...
| Side strip modes Neon/On/Breathing/Off | en side modes; `SIDE_LIGHT_EFFECTS` | side effect 1–4 | MATCH (live write verified) | | Live read bytes 24–31; live write and readback verified across modes 1..4 (2026-09-21); user visually confirmed green constant color. |
...
| Side brightness/speed/color | FUNC 25–31 | sliders + color | MATCH (live write verified) | | Live write and readback verified (2026-09-21). |
...
| Hub control / action | Vendor evidence | App path | Status | Semantic gap | Verification needed |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 4 layers Win / WinFn / Mac / MacFn | en `120`–`123` | Layer 0–3 toggles + read/apply | MATCH (live write verified) | App labels match. | Live write across all 4 layers verified with key slot 10 change and 512B readback, followed by clean restoration (2026-09-21). |
...
| Basic / Mouse / Media / Lighting / Extended palettes | en `106`–`108`, `remapKey.*`, chunk 1833 `eG()` | Exact G75 `eG()` sets: Basic (104), Mouse (7), Media (7/6), Main Lighting (9), Side Lighting (8), Extended (layer-aware WinExtra47 or MacExtra32 + SwitchProfile + Clear + base Fn) | MATCH (mock-tested; live slot remap verified) | Visible palette aligns 1:1 with chunk 1833 `eG()` and layer-aware Extra sets. Generic reset/battery/pairing correctly omitted from visible palette while preserving raw decode compatibility. | Layer 0..3 palette tests pass (185/184/169/168 items). Mock UI apply & readback verified; slot 10 physical remap write verified on hardware (2026-09-21). |
| Macro assign from remap | en `remapKey.macro` | Macros tab + Assign to Selected Key (type 112) | MATCH (live macro keybind verified) | Hub in-palette; app separate tab. Playback type 0/1/255 aligned with en `821`/`825`/`823`. | Slot 0 one-shot abc bound to Q across all 4 layers verified on hardware, producing actual 'abc' output (2026-09-21). |
...
| Hub control / action | Vendor evidence | App path | Status | Semantic gap | Verification needed |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 16 hardware slots | CMD 12/13; en macros | `readMacros` / `applyMacros` | MATCH (safe 4096B write verified) | Drafts require a successful CMD 12 parse (`hasReadMacros`). Failed reread, disconnect, and edit-target invalidate the writable bank. Apply freezes a hardware-safe snapshot (no extra IPC keys). | 8192-byte read window with writes restricted to first 4096 bytes and fresh readback verified (2026-09-21; offsets 4096 and above alias profile 0 colors/layers); slot 0 one-shot 'abc' bound to Q across all 4 layers confirmed via readback and trusted browser typing. Not all 16 slots or playback modes tested. |
...
| Bank capacity & deduplication | readable lines 36460–36524 reuses offsets for duplicate action bodies; `docs/MACRO_METADATA.md` | Vendor-identical action-body deduplication in serializer (`src/protocol.cjs`), validator (`src/schema-validators.cjs`), capacity checks (`src/macro-draft.js`), and renderer edits/recordings; wire delay 0..4 normalization; prospective bank checks account for release reservation | MATCH (safe 4096B write verified) | Reuses body offsets across slots independently of playback header. Preserves reserved bytes 50..63 from existing buffers. Prospective bank-state check blocks divergence overflow fail-closed on every edit and recording event. | Unit normalization & boundary tests; safe 4096-byte boundary and live write verified on hardware (2026-09-21). |
...
| Hub control / action | Vendor evidence | App path | Status | Semantic gap | Verification needed |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Polling rate | en `660`–`661`, `reportRateWarning`; `changeReportRate` | horizontal radios 1/2/4/8 kHz (`reporteRate` nibble), autosave on change | MATCH (live write verified) | G75 V2 supports 1k/2k/4k/8k (rates 4, 3, 2, 1). SafelyUpdateReportRate suppresses generic warning; none is shown. Unknown nibbles are labeled `Unknown (n)` and omitted from field saves so they are not rewritten to 1 kHz. Global Apply is hidden. An explicit Read from Device remains for recovery. | Live read byte 4; enum values 1..4 write and readback verified (2026-09-21). Actual physical polling Hz and latency not measured. |
| Debounce / “key combo mode” | en `664`–`665`; `iG` toggle `debounceLevel>=1` via `toggleKeyComboMode` | toggle reads `>=1`, writes `7`/`0` | MATCH (live write verified) | Hub toggle maps debounceLevel >= 1; writes 7 (on) or 0 (off). Preserves raw intermediate debounce (1..6) on read until explicit user toggle. | Live read byte 7; write and readback verified (2026-09-21). |
| Sleep time | en `3050`–`3052`; slider **60–1800 s**, step 60 | slider 1..30 min (wire units 30s) + Never Sleep; commit-only write | MATCH (live write verified) | Untouched raw `sleepTime` is labeled in real units (30 s, 2.5 min) without rounding the stored byte. Slider thumb may clamp to 1..30 min until the user adjusts. Unrelated field saves preserve the raw byte. | Live read bytes 35–36; write and readback verified; user confirmed 60s sleep and physical key wake (2026-09-21). |
| Never sleep | `toggleNeverSleep` / `sleepMode`; GLW `setSleepTime` `{sleepTime, sleepMode:0}` | checkbox + enabled slider | MATCH (live write verified) | Checkbox writes sleepMode 1/0 and preserves sleepTime. Display is 0 min while enabled; the slider stays usable. Changing the slider stages sleepMode 0 and unchecks Never Sleep. | Write and readback verified (2026-09-21). |
| Lock Win | en `666`–`667`, `890` disabled in Mac | checkbox; disabled in Mac mode (translation 890) | MATCH (live write verified) | Disabled in staged Mac mode; switching to Mac clears lockWin to false. | Write and readback verified (2026-09-21). |
| Mac mode | en `670` | OS Mode buttons | MATCH (live write verified) | Switches layer and macMode byte 1; switching to Mac stages lockWin=false. | Live read byte 1; write and readback verified (2026-09-21). |
...
3. **Profile ecosystem remaining:** game/app auto-bind is a host-side bundle-id table (mock-tested). Physical profile slot enable (3->4) and activation switch (1->0) were verified on hardware (2026-09-21); custom onboard name slot writes (CMD 242) and cloud/network libraries remain unproven. Local library, onboard names (device-stored `i18n<defaultOnboard>` tokens display as `Default Onboard`/`2`/`3`; empty storage still translated locally; 16-char displays allocate unique 2–15 local names), tE/tI/tk/tg/tL, and official v3 file import/export/apply roundtrip are implemented and verified. Cloud/official/Apex **network** libraries and share-codes remain missing. Translation 870’s “six configurations” is stale; ordinary capacity is 20.
...
7. **Macros:** 16-slot editor, pause/resume, validated Add/Insert/Replace honoring palette kinds and consistent keypress pairs, copy/paste, atomic canonical metadata persistence, recorder release-reservation, vendor-identical action-body deduplication (independent playback headers, wire delay 0..4 normalization, reserved bytes 50..63 preservation), and prospective bank-state preflight checks are implemented and mock/unit-tested. 8192-byte read window with writes restricted to first 4096 bytes and fresh readback verified on hardware (offsets 4096 and above alias profile 0 colors/layers, 2026-09-21); slot 0 one-shot 'abc' bound to Q across all 4 layers verified outputting 'abc' with browser trusted KeyA/B/C events; not all 16 slots or playback modes tested.
8. **Performance:** two-column iG `et` cards, horizontal 1/2/4/8 kHz radios, commit-only sleep, Never Sleep resume, combo toggle, Mac/Win-lock coupling, and per-field autosave are implemented and verified. Global Apply is hidden. An explicit Read from Device remains (recovery merge, not vendor-identical automatic write). Local preview still requires a connected device. Parameter writes (sleep, mode, debounce, Win lock, OS mode, rate enums 1..4) and 60s sleep/wake confirmed on hardware (2026-09-21); actual physical polling rate (Hz) and latency were not measured.
9. **Physical hardware verification (2026-09-21):** Seven configuration paths have been verified via live write and fresh readback on a physical 2.4G receiver (VID `0x3837`, PID `0x3033`, MCU FW 1.14 / RF 1.30): onboard profiles (enable 3->4, activate 1->0, restored to 3), all 4 keymap layers (slot 10 write/readback/restore), main RGB lighting IDs 0..22 (22 presets + custom, only constant red visually confirmed), side lighting 1..4 (green constant visual confirmed), 8192-byte read / safe 4096-byte write macros (slot 0 one-shot 'abc' on Q confirmed with real typing output and browser trusted events), performance and sleep (sleep time/mode, debounce, lock Win, OS mode, polling rate enums 1..4; 60s sleep and wake confirmed), and profile export/import/apply roundtrip (native and official v3; each export backs up one profile, not the whole keyboard). All 63 memory regions cleanly restored to baseline (0 mismatches, excluding only live FUNC bytes 32/34 battery/status). What remains unverified on physical hardware: actual polling frequencies/latency measurements, all lighting animation patterns visually inspected, all 16 macro slots/modes, MT/TGL/SOCD/CB advanced physical mutations, factory reset, firmware flashing, and wired USB mode.
...
- `npm test` (551 logical unit/mock tests) / `npm run test:mock-ui`: protocol framing, validators, in-memory GLW, renderer races.
- Physical hardware verification (2026-09-21, 2.4G receiver VID `0x3837`, PID `0x3033`, MCU FW 1.14 / RF 1.30): verified live writes, fresh readbacks, specified lighting and physical key observations, and clean 63-region baseline restoration (excluding live battery/status bytes 32/34) across seven core configuration paths (profiles, 4 layers, main RGB lighting IDs 0..22 [22 presets + custom], side lighting 1..4, safe 4KB macros, performance/sleep, native/v3 import-export apply).
- Still unverified on physical hardware: factory reset, live firmware flash, wired USB mode, advanced MT/TGL/SOCD/CB physical execution, GIF host streaming, and physical polling Hz / latency measurements.
