...
## Deduplicated capacity and conservative writable boundary

Official `setMacros` near readable lines 36460–36524 reuses offsets for duplicate action bodies (lookup table `s[m]`, where body identity is derived from the macro action list), while playback remains per-slot header data. The serializer `serializeMacroRegion`, validator `validateMacroSlots`, and editor prospective capacity helpers enforce a conservative 4096-byte writable limit (`MACRO_WRITABLE_LIMIT`) with identical action-body deduplication. While the physical read window is 8192 bytes (`MACRO_READ_WINDOW_SIZE`), physical testing confirmed that the tail region is aliased to other hardware banks: `macros[4096..6143]` mirrors profile 0 RGB key colors, and `macros[6144..8191]` mirrors profile 0 keylayers. Writing beyond 4096 via CMD 13 clobbers these regions; therefore, writable allocations, parser offset bounds, and transport write commands strictly bound macro write operations to the safe 4096-byte prefix.
