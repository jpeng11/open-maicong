...
Flag item _isMatch (23351–23398) accepts first payload bytes00 00, rejects00 01 with receive flag1, and ignores reports whose first byte is nonzero. It does not match an opcode/offset, so firmware transfers must remain strictly serialized and identity-bound. SDK autoCompleteDelay resolves without a reply for enterBoot/end/success at16ms and erase at5000ms. Those values are host-side queue conveniences: they are not evidence of device completion, and they are not a failure deadline. Treating the erase 5000ms figure as a hard timeout that declares the erase failed is a stronger and more dangerous use of that number than the SDK made of it. A standalone updater must report timeout/uncertainty honestly and use verified chunk comparison plus return-to-normal identity as completion evidence. Do not copy the SDK generic retry defaults onto uncertain erase commands, and do not promote autoCompleteDelay into a destructive-path deadline.
...
sendRomData/checkRomData at byte61403 split the supplied complete file into32-byte chunks at offsets32*index. Both official catalog images have a remainder after that split: keyboard 264408 = 32×8262 + 24, receiver 121032 = 32×3782 + 8. The SDK path is a JavaScript `slice` of the remaining bytes, so the last packet's length field would be that remainder and the unused report bytes would be the same zero pad used for every flag-response packet (packetLength64). That short-tail framing is inferred from SDK slice behaviour; no short-tail write or verify packet has been captured on the wire. This is a destructive path. Do not treat the inferred tail as observed device behaviour. Check opcode130 resends the expected chunk for device-side comparison and expects the flag response; it is not a raw flash readback. No header stripping or version parsing is performed by this call path. The receiver header version discrepancy therefore remains unresolved; do not label its byte4–5 as the displayed RF firmware version.
...
Location continuity across actual G75 boot transition has NOT been tested. It identifies a USB port/topology position, not cryptographic physical-device identity. Continuity still requires a bounded transition window, exact collection matching, fail-closed ambiguity handling, and serial equality only when both identities expose a serial. A missing serial is an absent field, not a mismatch: boot descriptors commonly ship a minimal string set with no serial. Requiring serial equality whenever either side has one rejects that common case. No boot-mode command was issued for this observation.
