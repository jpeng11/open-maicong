...
- 2026-09-21 实物验证：在真实 2.4G 接收器（VID `0x3837`、PID `0x3033`、MCU 固件 1.14、RF 固件 1.30）上完成了 7 项配置路径的实机写入与全新回读核实（551 项逻辑单元测试与 Electron 界面模拟测试通过）：
  1. **板载配置**：第 4 配置槽启用（3->4）、激活配置切换（1->0）及即时回读，测试后已完整复原为 3 个配置且槽位顺序完好。
  2. **四层按键映射**：Win、WinFn、Mac、MacFn 全部 4 层按键槽位修改、512 字节回读校验及干净复原。
  3. **RGB 主灯效**：主灯效 ID 0–22（22 种预设与自定义）配置写入与回读校验已核实。仅人工目视核验常亮红光，其余动画未逐一目视验收。
  4. **侧灯指示**：模式 1–4 与参数写入及回读校验，人工目视核验常亮绿光。
  5. **硬件宏**：8192 字节读取窗口且仅写入前 4096 字节与全新回读已核实（偏移量 4096 及以上别名映射第 0 配置色彩与图层），槽 0 单次触发 abc 绑定至 Q 键实测打字输出 abc 并捕获受信任按键事件（未测试全部 16 槽或全部回放模式）。
  6. **性能与休眠**：休眠时间/模式、去抖、锁 Win、系统模式及回报率枚举值（1..4）写入及回读校验，人工确认 60 秒休眠与物理按键唤醒（未测定实际物理回报率与输入延迟）。
  7. **配置导入导出**：原生 JSON 与官方 v3 导出、导入、实机应用及重导出验证，全 4 层、83 颗按键 LED、设置与宏完全匹配。原生格式保留更多配置字段，每次导出仅备份单个配置而非整把键盘；官方格式省略校准与 keyExtras 且应用时保留实机现有值。写入保持非原子性（分段失败无事务回滚）。
  测试后已完整恢复全部 63 个内存区域（0 差异，仅忽略易变的电量与状态字节 32/34）。
- 未在实机上执行的项目保持原有验证状态：高级功能（MT/TGL/SOCD/CB）、GIF 主机串流、白平衡校准、恢复出厂设置物理执行、固件物理烧录以及有线 USB-C 模式仍为模拟验证或未在实机上进行破坏性测试。在对键盘进行大范围改动前，建议先导出当前配置备份文件。
